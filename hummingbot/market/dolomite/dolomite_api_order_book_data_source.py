...
class DolomiteAPIOrderBookDataSource(OrderBookTrackerDataSource):
    ...
    async def get_active_exchange_markets(cls) -> pd.DataFrame:
        ...
        async with aiohttp.ClientSession() as client:
            markets_response: aiohttp.ClientResponse = await client.get(MARKETS_URL);
    ...
    async def get_snapshot(self, client: aiohttp.ClientSession, trading_pair: str, level: int = 3) -> Dict[str, any]:
        async with client.get(SNAPSHOT_URL.replace(':trading_pair', trading_pair)) as response:
        ...
        async with aiohttp.ClientSession() as client:
            ...
            for index, trading_pair in enumerate(trading_pairs):
                try:
                    ...
                    self.logger().error(f"Error getting snapshot for {trading_pair} in get_tracking_pairs.", exc_info=True)
